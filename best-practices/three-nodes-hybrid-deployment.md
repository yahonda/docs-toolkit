...
When TiKV detects that the number of CPU cores of the machine is greater than or equal to `16`, this parameter value defaults to `8`. When the number of CPU cores is smaller than `16`, the parameter value defaults to `4`. This parameter is used when TiKV converts complex transaction requests to simple key-value reads or writes, but the scheduler thread pool does not perform any writes.
